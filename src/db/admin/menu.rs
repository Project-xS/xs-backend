...
impl MenuOperations {
    ...
    pub fn add_menu_item(&self, menu_item: NewMenuItem) -> Result<MenuItem, RepositoryError> {
        ...
        diesel::insert_into(menu_items)
            ...
            .map_err(|e| {
                error!("add_menu_item: error inserting menu item '{}': {}", menu_item.name, e);
    ...
    pub fn remove_menu_item(&self, id: i32) -> Result<MenuItem, RepositoryError> {
        let mut conn = DbConnection::new(&self.pool).map_err(|e| {
            error!("remove_menu_item: failed to acquire DB connection for id {}: {}", id, e);
            e
        })?;
        ...
        diesel::delete(menu_items.filter(item_id.eq(id)))
            .get_result(conn.connection())
            .map_err(|e| {
                error!("remove_menu_item: error deleting menu item with id {}: {}", id, e);
                match e {
    ...
    pub fn update_menu_item(
        ...
        let mut conn = DbConnection::new(&self.pool).map_err(|e| {
            error!("update_menu_item: failed to acquire DB connection for id {}: {}", itemid, e);
            e
        })?;
        ...
        diesel::update(menu_items.filter(item_id.eq(itemid)))
            ...
            .get_result(conn.connection())
            .map_err(|e| {
                error!("update_menu_item: error updating menu item with id {}: {}", itemid, e);
                match e {
    ...
    pub fn get_menu_item(&self, itemid: i32) -> Result<MenuItem, RepositoryError> {
        let mut conn = DbConnection::new(&self.pool).map_err(|e| {
            error!("get_menu_item: failed to acquire DB connection for id {}: {}", itemid, e);
            e
        })?;
        ...
        menu_items
            ...
            .get_result(conn.connection())
            .map_err(|e| {
                error!("get_menu_item: error fetching menu item with id {}: {}", itemid, e);
                match e {
                    Error::NotFound => RepositoryError::NotFound(format!("menu_items: {}", itemid)),
                    other => RepositoryError::DatabaseError(other),
                }
